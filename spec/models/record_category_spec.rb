...
describe RecordCategory do
  ...
  describe '#status' do
    ...
    it 'summarizes status' do
      ...
      status[:duration].should == 2 * 60 * 60 # 2 hours
      status[:recent_entries].length.should == 2
    ...
    it 'includes subcategories'  do
      ...
      status[:duration].should == 1.hour
      status[:recent_entries].length.should == 1
    end
    ...
    it 'handles midnight spans' do
      ...
      status = cat.status
      status[:duration].should == 8.hours
    end
  end
