...
describe Record, type: :model do
  ...
  describe '#data=' do
    before :each do
      @x = FactoryGirl.create(:record, user: @user, record_category: @cat)
    end
  ...
  describe '#end_timestamp_must_be_after_start' do
    it "invalidates records" do
      x = FactoryGirl.build(:record, timestamp: Time.zone.now, end_timestamp: Time.zone.now - 1.hour, user: @user, record_category: @cat)
      expect(x).to_not be_valid
    end
  end
  describe '.split' do
    context 'when given a record-type record' do
      it "does not split, even if there's an end timestamp" do
        record_cat = FactoryGirl.create(:record_category, user: @user, category_type: 'record')
        x = FactoryGirl.create(:record, timestamp: Time.zone.now.midnight - 1.hour, end_timestamp: Time.zone.now.midnight + 25.hours, record_category: record_cat)
        expect(Record.split(x)).to eq [x]
      end
   end
    context 'when given an activity record' do
      it "handles same-day events" do
        x = FactoryGirl.create(:record, timestamp: Time.zone.now.midnight + 1.hour, end_timestamp: Time.zone.now.midnight + 2.hours, user: @user, record_category: @cat)
        expect(Record.split(x)).to eq [x]
      end
      ...
      it "handles events that cross two midnight boundaries" do
        x = FactoryGirl.create(:record, timestamp: Time.zone.now.midnight - 1.hour - 1.day, end_timestamp: Time.zone.now.midnight + 2.hours, user: @user, record_category: @cat)
        split = Record.split(x)
  ...
  describe '#add_data' do
    context 'when there is no end time but there is a next activity' do
      it "automatically sets the timestamp" do
        r1 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now - 30.minutes)
  ...
  describe '#update_previous' do
    it "adjusts the previous activity's ending timestamp" do
      r1 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now)
      r2 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now - 1.hour)
  ...
  describe '#duration' do 
    it 'reports proper duration if the ending timestamp is set' do
      r1 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now - 2.hours, end_timestamp: Time.zone.now - 1.hour)
      r1.calculated_duration.should == 1.hour
    end
    it 'truncates to current timestamp if the ending timestamp is not set' do
      r1 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now - 2.hours)
  ...
  describe '.recalculate_durations' do
    it "fixes all the durations" do
      r0 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now.yesterday.midnight + 1.hour)
      r1 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now.yesterday.midnight + 45.minutes)
      r2 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now.yesterday.midnight + 15.minutes)
      r3 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now.yesterday.midnight + 30.minutes)
      r3.destroy
      Record.recalculate_durations(@user)
  ...
  describe '#private?' do
    it "recognizes private notes" do
      r3 = FactoryGirl.create(:record, user: @user, record_category: @cat, data: {'note' => 'this is !private'})
      expect(r3).to be_private
    end
    it "recognizes public notes" do
      r3 = FactoryGirl.create(:record, user: @user, record_category: @cat, data: {'note' => 'this is public'})
  ...
  describe '#current_activity' do
    ...
    context "when this is not an activity" do
      it "returns the enclosing activity if there is one that hasn't ended yet" do
        r0 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now.yesterday.midnight + 1.hour)
        r1 = FactoryGirl.create(:record, user: @user, record_category: @record_cat, timestamp: Time.zone.now)
        expect(r1.current_activity).to eq r0
      end
      it "returns the enclosing activity if there is one that spans this" do
        c2 = FactoryGirl.create(:record_category, user: @user, category_type: 'record')
        r0 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now.yesterday.midnight + 1.hour, end_timestamp: Time.zone.now.yesterday.midnight + 2.hours)
        r1 = FactoryGirl.create(:record, user: @user, record_category: @record_cat, timestamp: Time.zone.now.yesterday.midnight + 90.minutes)
        expect(r1.current_activity).to eq r0
      end
  ...
  describe '#context' do
    it "returns the activity's information" do
      r0 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now.yesterday.midnight + 1.hour, end_timestamp: Time.zone.now.yesterday.midnight + 2.hours)
      r1 = FactoryGirl.create(:record, user: @user, record_category: @record_cat, timestamp: Time.zone.now.yesterday.midnight + 90.minutes)
      r2 = FactoryGirl.create(:record, user: @user, record_category: @record_cat, timestamp: Time.zone.now.yesterday.midnight + 89.minutes)
      r3 = FactoryGirl.create(:record, user: @user, record_category: @record_cat, timestamp: Time.zone.now.yesterday.midnight + 91.minutes)
      r4 = FactoryGirl.create(:record, user: @user, record_category: @cat, timestamp: Time.zone.now)
      x = r1.context
  ...
  describe '.get_zoom_key' do
    it 'gets the day' do
      expect(Record.get_zoom_key(@user, :daily, Time.zone.local(2013, 1, 2, 3, 4)).to_date).to eq Time.zone.local(2013, 1, 2).to_date
    end
    it 'gets the end of the week' do
      expect(Record.get_zoom_key(@user, :weekly, Time.zone.local(2013, 1, 2, 3, 4)).to_date).to eq Time.zone.local(2013, 1, 4).to_date
    end
    it 'gets the start of the month' do
      expect(Record.get_zoom_key(@user, :monthly, Time.zone.local(2013, 1, 2, 3, 4)).to_date).to eq Time.zone.local(2013, 1, 1).to_date
    end
    it 'gets the start of the year' do
      expect(Record.get_zoom_key(@user, :yearly, Time.zone.local(2013, 2, 2, 3, 4)).to_date).to eq Time.zone.local(2013, 1, 1).to_date
    end
  end

  describe '#get_zoom_key' do
    it 'gets the day' do
      f = FactoryGirl.create(:record, user: @user, timestamp: Time.zone.local(2013, 1, 2, 3, 4), record_category: @cat)
  ...
  describe '.refresh_from_tap_log' do
    ...
    it "loads the information" do
      ...
      Record.refresh_from_tap_log(@user3, file)
      expect(@user3.records.size).to eq 2
      expect(@user3.records[0].record_category.full_name).to eq 'Personal - Routines'
      expect(@user3.records[1].record_category.full_name).to eq 'Discretionary - Gardening'
    end
...
10:00 J
END
      out = Record.confirm_batch(@user2, lines)
      Record.create_batch(@user2, out)
      expect(@user2.records.length).to eq 2
    end
    it 'sets the end if specified' do
      prev_rec = FactoryGirl.create(:record, user: @user2, record_category: @cat_abcx, timestamp: Time.zone.now.midnight)
      next_rec = FactoryGirl.create(:record, user: @user2, record_category: @cat_abcx, timestamp: Time.zone.now.tomorrow.midnight)
      lines = <<END
7:30 ABC
8:30 9:00 XYZ
9:30 X
...
END
      out = Record.confirm_batch(@user2, lines)
      Record.create_batch(@user2, out, set_end: true)
  ...
  describe '.parse' do
    before(:each) do 
      @notes_cat = FactoryGirl.create(:record_category, user: @user, name: 'ABC', category_type: 'activity', data: [{'key' => 'notes', 'label' => 'Notes', 'type' => 'end'}])
    end
    context 'when the category is specified' do
      it "gets the category" do
        x = Record.parse(@user, category: '8:00 ABC')
        expect(x.record_category).to eq @notes_cat
    ...
    context 'when the category ID is specified' do
      it "uses that ID" do
        x = Record.parse(@user, category_id: @notes_cat.id, category: '8:00')
        expect(x.record_category).to eq @notes_cat
        expect(x.timestamp.hour).to eq 8
      end
    end
  ...
  describe '.get_records' do
    before(:each) do
      @note_cat = FactoryGirl.create(:record_category, category_type: 'activity', user: @user, data: [{'key' => 'note', 'label' => 'Note', 'type' => 'text'}])
      # 2012-01-02 8:00
      FactoryGirl.create(:record, record_category: @note_cat, user: @user, timestamp: Time.zone.local(2012, 1, 2, 8))
      # 2012-01-02 9:00
      FactoryGirl.create(:record, record_category: @note_cat, user: @user, timestamp: Time.zone.local(2012, 1, 2, 9), data: {'note' => 'search'})
      # 2012-01-03 10:00 private
      FactoryGirl.create(:record, record_category: @note_cat, user: @user, timestamp: Time.zone.local(2012, 1, 2, 10), data: {'note' => 'this is !private'})
    end
  ...
  describe '#beginning_of_week' do
    it "returns the beginning" do
      rec = FactoryGirl.create(:record, timestamp: Time.zone.local(2012, 2, 2, 8), end_timestamp: Time.zone.local(2012, 2, 2, 9), source_name: 'ruby', source_id: 1, data: {'note' => 'stuff'}, user: @user, record_category: @cat)
